...
function leastCommonFlavor(candyBag) {
  // Your code here
    let flavorLeastCommon = '';
    let candyAmount = Infinity;

    //This if statment will check if the object is empty
    if(Object.keys(candyBag).length === 0){
        return null;
    }

    for (let candy in candyBag){
        const candyCurrAmount = candyBag[candy];
        // console.log(candyCurrAmount)
        if(candyCurrAmount < candyAmount) {
            candyAmount = candyCurrAmount;
            flavorLeastCommon = candy;
        }
    }
return flavorLeastCommon;
};
