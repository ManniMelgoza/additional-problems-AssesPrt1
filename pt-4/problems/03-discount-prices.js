...
*/
discountPrices = (prices, dicounts) => {

//    return prices.map(prices, index) => {

//    }

    let newDiscountPrice = [];
    for (let price of prices){
        for (let discount of dicounts){
            let totalDisocunt = (discount * price);
            let newPrice = (price - totalDisocunt).toFixed(2);
            newDiscountPrice.push(`$${newPrice}`);
        }
